...
export async function fuzz(options: FuzzOptions): Promise<FuzzResult> {
  ...
  // start fuzzing
  const fuzzResult = await core.group("fuzzing", async () => {
    const args = [
      "test",
    ...
    return await exec.getExecOutput("go", args, ignoreReturnCode);
  });

  if (fuzzResult.exitCode === 0) {
  ...
  const report = await core.group("generating report", async () => {
    return await generateReport(options, fuzzResult.stdout);
  });
  if (report == null) {
...
interface GenerateReportResult {
  ...
  // fuzzResult is the output of the fuzz test.
  fuzzResult: string;
...
async function generateReport(options: FuzzOptions, fuzzOutput: string): Promise<GenerateReportResult | null> {
  const ignoreReturnCode = { cwd: options.workingDirectory, ignoreReturnCode: true };

  const lines = fuzzOutput.split("\n");
  const failIndex = lines.findIndex((line) => line.startsWith("--- FAIL:"));
  const fuzzResult = failIndex >= 0 ? lines.slice(failIndex).join("\n") : "";
...
async function sendReportViaPR(options: FuzzOptions, report: GenerateReportResult): Promise<FuzzResult> {
  ...
  await createCommit(client, options, {
    ...
    message: {
      ...
      body: `fuzzing failed with the following output:

${"```"}
${report.fuzzResult}
${"```"}

The result of ${"`"}${report.testCommand}${"`"} is:
